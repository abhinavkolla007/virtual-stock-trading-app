package com.abhi.virtualstock.service;
...
import com.abhi.virtualstock.constants.Constants.StockExchange;
import com.abhi.virtualstock.dto.PriceResponse;
import com.abhi.virtualstock.dto.StocksListResponse;
import com.abhi.virtualstock.dto.TradeOrder;
import com.abhi.virtualstock.exception.StockRecommendationException;
import com.abhi.virtualstock.model.StockHolding;
import com.abhi.virtualstock.model.StockInfo;
import com.abhi.virtualstock.model.Transaction;
import com.abhi.virtualstock.repository.StockHoldingRepository;
import com.abhi.virtualstock.repository.StockInfoRepository;
...
public class StockService {
    ...
    public void fetchStocksOnStartup() {
        startRetrievingStocks(); // calls the same method that the scheduled job would call
    }
    @Value("${rapid.api.key}")
    ...
    try {
        ...
        // Build the request URL for Twelve Data
        String url = "https://" + apiHost + "/price?symbol=" + symbol;
        ...
        if (responseEntity.hasBody() && responseEntity.getBody() != null) {
            Object priceObj = responseEntity.getBody().get("price");
            if (priceObj != null) {
    ...
    throw new Exception("Couldn't fetch the current stock price for " + symbol);
}
