...
import com.abhi.virtualstock.dto.NewsArticleCommentDto;
import com.abhi.virtualstock.dto.NewsArticleDto;
import com.abhi.virtualstock.dto.NewsListResponse;
import com.abhi.virtualstock.exception.InvalidArticleException;
import com.abhi.virtualstock.mapper.NewsArticleMapper;
...
import com.abhi.virtualstock.repository.NewsArticleCommentRepository;
import com.abhi.virtualstock.repository.NewsRepository;
    ...
    for (NewsArticle article: newsArticles) {
        try {
            if (!newsRepository.existsByTitle(article.getTitle())) {
                // TRUNCATE description to 65535 characters (MySQL TEXT max)
                if (article.getDescription() != null && article.getDescription().length() > 65535) {
                    article.setDescription(article.getDescription().substring(0, 65535));
                }
