...
export default function StockDetails({ stock, handleFormatQuote }) {
  ...
  useEffect(() => {
    const fetchStockQuoteAndLogo = async () => {
      ...
      try {
        // Fetch quote from Twelve Data (free plan) via RapidAPI
        const quoteResponse = await fetch(
          `https://twelve-data1.p.rapidapi.com/quote?symbol=${stock.symbol}`,
          {
            method: "GET",
            headers: {
        ...
        if (quoteResponse.ok) {
          ...
          // Twelve Data returns fields like: open, high, low, close, previous_close, change, percent_change, volume
          if (!data || Object.keys(data).length === 0 || data.status === "error") {
          ...
          const formattedQuote = {
            ...
            bidPrice: price - 0.01,
            askPrice: price + 0.01,
            volume: data.volume !== undefined ? parseFloat(data.volume) : null,
            prevClose: data.previous_close !== undefined ? parseFloat(data.previous_close) : null,
            change: data.change !== undefined ? data.change : null,
            changePercent: data.percent_change !== undefined ? `${data.percent_change}%` : null,
          };

          // Fetch company logo (also Twelve Data)
